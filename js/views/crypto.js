...
function cryptoRelated(c) {
  ...
  return { panta: [...Panta.markets.values()].filter(m => !m.cancelled && hit(m.title)).slice(0, 8), poly: Listings.all().filter(m => hit(m.q) && !Listings.pantaFor(m)).sort((a, b) => b.vol24 - a.vol24).slice(0, 6) };
}
function pantaRow(m) {
...
function polyRow(m) { if (m.listed) return `<div class="mrow"><a class="mrow-l" href="#/market/${m.id}"><b>${esc(m.q)}</b><span class="mut" style="font-size:11.5px">New · the first trade opens it on Panta</span></a><div class="mrow-p"><span class="num" data-lpct="${m.id}">${Math.round(m.yes * 100)}%</span></div><a class="btn btn-ghost sm" href="#/market/${m.id}">Trade</a><span class="mrow-v num mut">est.</span></div>`; return `<div class="mrow"><a class="mrow-l" href="#/polymarket/${m.id}"><b>${esc(m.q)}</b><span class="mut" style="font-size:11.5px">Polymarket · trade in Nexis</span></a><div class="mrow-p"><span class="num" data-lpct="${m.id}">${Math.round(m.yes * 100)}%</span></div><a class="btn btn-ghost sm" href="#/polymarket/${m.id}">Trade</a><span class="mrow-v num mut">${kusd(m.vol)}</span></div>`; }
function relatedMarketsCard(rel, emptyText) {
  ...
  return `<div class="card"><div class="card-head"><h3>Related prediction markets</h3>${srcBadge('panta', true)}</div>
    ...
    ${rel.poly.length ? `<div class="card-head" style="border-top:1px solid var(--line)"><h3 style="font-size:13px">${rel.pmEvent ? 'This game on Polymarket' : 'More markets'}</h3>${rel.pmEvent ? srcBadge('pmtrade', true) : ''}</div>${rel.poly.map(polyRow).join('')}` : ''}</div>`;
}
