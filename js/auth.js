...
function integrationsHtml() {
  ...
  return `<div class="card"><div class="card-head"><h3>${ic('plug', 'sm')}Server integrations</h3><button class="btn btn-ghost sm" data-action="feedStatus">Live status</button></div>
    ${c.error ? `<div class="card-pad">${unavailable('Server functions unreachable', 'This copy of Nexis isn’t running with its /api functions (for example, opened as a local file or inside a preview). Deploy to Vercel to enable integrations.')}</div>` : ''}
