...
const A_APP = {
  ...
  bkTab: (el) => { UI.book.tab = el.dataset.v; UI.book.limit = 80; refresh(); },
  bkWhen: (el) => { UI.book.when = el.dataset.v; UI.book.limit = 60; refresh(); },
  bkMore: () => { UI.book.limit += 80; refresh(); },
  bkClearQ: () => { UI.book.q = ''; refresh(); },
  bkPick: (el) => { Book.toggle(el.dataset.k, { label: el.dataset.l, market: el.dataset.mk, game: (() => { const g = Book.get(el.dataset.g); return g ? `${g.home.short} vs ${g.away.short}` : ''; })() }); },
  bkBt: (el) => { UI.book.bt = el.dataset.v; refresh(); },
  bkGcat: (el) => { UI.book.gcat = el.dataset.v; refresh(); },
  bkSlipMode: (el) => Book.setMode(el.dataset.v),
  bkAskYes: () => bkAnswer(true),
  bkAskNo: () => bkAnswer(false),
  bkConfirm: (el) => { el.disabled = true; bkConfirm(); },
