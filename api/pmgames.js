...
// 8 hours ago and 21 days from now. The browser matches them to ESPN games by team and time.
...
module.exports = async (req, res) => {
  ...
  const lo = Date.now() - 8 * 3600e3, hi = Date.now() + 21 * 86400e3; const seen = new Set(); const events = [];
  raw.forEach(([e, league]) => {
    if (!e || seen.has(String(e.id)) || !isGame(e.title)) return; seen.add(String(e.id));
    const t = trimEvent(e, league); if (t && !t.ended && t.start && t.start >= lo && t.start <= hi) events.push(t);
  });
